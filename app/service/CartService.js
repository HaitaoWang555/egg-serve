...
class CartServiceService extends Service {
  /**
  ...
  */
  ...
  async deleteCartByproductIdList({ productIdList }) {
    ...
    const deleteCount = await this.CartModel.destroy({ where: { userId, productId: { [Op.in]: productIdArr } } });
  ...
  async getCartProductCount() {
    ...
    const [ countRow ] = await this.CartModel.findAll({
      where: { userId },
      attributes: [
        [ this.Sequelize.fn('sum', this.Sequelize.col('quantity')), 'count' ],
      ],
    });
...
module.exports = CartServiceService;
