...
class OrderServiceService extends Service {
/**
...
*/
  ...
  async _reduceUpdateProductStock(orderItemList) {
    orderItemList.forEach(async item => {
      await this.ProductModel.update({ stock: this.ctx.app.Sequelize.literal(`stock - ${item.quantity}`) }, { where: { id: item.productId } });
    });
  }
